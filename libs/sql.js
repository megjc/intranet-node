let statements = {
    // GETBYTYPE: 'SELECT id, first_name, last_name, middle_name, division,'
    //             + 'affiliation, position, start_date, end_date FROM employees WHERE ?? = ?',
    ...
    GET: 'SELECT employees.id, employees.name, employees.position, employees.classification_id, allowances.title, activities.title AS `act_title`, allowances.amount FROM employees'
          ...
          + ' INNER JOIN activities ON employees.activity_id = activities.id ORDER BY activities.title ASC',
    ...
    EXPIREDDOCS: "SELECT d.id, d.title, d.expiry_date, d.employee_id, e.name, e.email FROM documents AS `d` INNER JOIN employees as `e` ON e.id = d.employee_id WHERE d.expiry_date <= NOW() AND d.expiry_date != '2000-01-01 00:00:00' ORDER BY e.name ASC",
...
// EXPIREDDOCS: 'SELECT d.id, d.title, d.expiry_date, d.employee_id, e.name FROM documents AS `d` INNER JOIN employees as `e` ON e.id = d.employee_id WHERE d.expiry_date <= NOW()',
